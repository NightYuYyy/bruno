...
const ImportWorkspace = ({ onClose }) => {
  ...
  const validateAndGetFilePath = (file) => {
    ...
    if (!isZip) {
      toast.error(t('WORKSPACE.INVALID_ZIP'));
      return null;
    }

    const filePath = window?.ipcRenderer?.getFilePath(file);
    if (!filePath) {
      toast.error(t('WORKSPACE.FILE_PATH_UNAVAILABLE'));
      return null;
    }
  ...
  return (
    <Modal
      ...
      confirmText={isSubmitting ? t('WORKSPACE.IMPORTING') : t('COMMON.IMPORT')}
    ...
    >
      <div className="flex flex-col">
        <div className="mb-4">
          <h3 className="font-semibold mb-2">{t('WORKSPACE.WORKSPACE_FILE')}</h3>
          {selectedFile ? (
            ...
            >
              <div className="flex flex-col items-center justify-center">
                ...
                <p className="text-gray-600 dark:text-gray-300 mb-2">
                  {t('WORKSPACE.DROP_ZIP_OR_CHOOSE')}{' '}
                  <button
                    type="button"
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  {t('WORKSPACE.SUPPORTS_WORKSPACE_ZIP')}
                </p>
              </div>
            </div>
          )}
        ...
        <div className="mb-4">
          <label htmlFor="workspace-location" className="font-semibold mb-2 flex items-center">
            ...
            <Help>
              <p>{t('WORKSPACE.EXTRACT_LOCATION_HELP_1')}</p>
              <p className="mt-2">
