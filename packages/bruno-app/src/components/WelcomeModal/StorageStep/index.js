...
const StorageStep = ({ collectionLocation, onBrowse }) => {
  ...
  return (
    <StyledWrapper className="step-body">
      <div className="step-label">{t('WELCOME.STORAGE_LABEL')}</div>
      <div className="step-title">{t('WELCOME.STORAGE_TITLE')}</div>
      <div className="step-description">
        {t('WELCOME.STORAGE_DESCRIPTION')}
      </div>
      ...
      <div className="location-input-group">
        ...
        >
          {collectionLocation ? (
            ...
            <span className="path-text path-placeholder">{t('WELCOME.CHOOSE_FOLDER_PLACEHOLDER')}</span>
          )}
