      ...
      {collectionImportPaths && collectionImportPaths.length > 0 && (
        <div className="content-wrapper">
          <div className="header-wrapper">
            <div className="header-text">{t('GRPC.FROM_COLLECTION_SETTINGS')}</div>
            <button
          ...
          {invalidImportPaths.length > 0 && (
            <div className="error-wrapper">
              <p className="error-text">
                ...
                {t('GRPC.SOME_IMPORT_PATHS_NOT_FOUND')}
                {' '}
                <button
                  onClick={onOpenCollectionProtobufSettings}
                  className="error-link"
                >
                  {t('GRPC.MANAGE_IMPORT_PATHS')}
                </button>
              </p>
            </div>
          )}
                    ...
                    <div className="item-left">
                      <div className="checkbox-wrapper">
                        <input
                          ...
                          className="checkbox"
                          title={importPath.enabled ? t('COLLECTION_SETTINGS.PROTOBUF.ENABLE_IMPORT_PATH') : t('COLLECTION_SETTINGS.PROTOBUF.DISABLE_IMPORT_PATH')}
                        />
                      </div>
      ...
      {(!collectionImportPaths || collectionImportPaths.length === 0) && (
        <div className="empty-wrapper">
          <div className="empty-text">
            {t('GRPC.NO_IMPORT_PATHS_CONFIGURED')}
          </div>
        </div>
      )}
