    ...
    <div className="collapsible-section mt-2">
      <div className="section-header" onClick={() => toggleHeaders(!areHeadersCollapsed)}>
        <pre className="flex flex-row items-center">
          <div className="opacity-70">{areHeadersCollapsed ? '▼' : '▶'}</div> {t('RESPONSE_PANE.TIMELINE.HEADERS')}
      ...
      {areHeadersCollapsed && (
        <div className="mt-1">
          {headers && Object.keys(headers).length > 0
            ...
            : <div className="timeline-item-timestamp">{t('RESPONSE_PANE.TIMELINE.NO_HEADERS_FOUND')}</div>}
        </div>
      )}
    </div>
  );
